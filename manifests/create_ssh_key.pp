...
define sshkeys::create_ssh_key (
  Variant[Undef,String] $passphrase    = undef,
  Variant[Undef,String] $owner         = undef,
  Variant[Undef,String] $group         = undef,
  Variant[Undef,String] $ssh_dir       = undef,
  Boolean $create_ssh_dir              = true,
  String $ssh_keytype                  = 'rsa',
  Variant[Undef,String] $ssh_bitlength = undef,
  Variant[Undef,Boolean] $require      = undef,
) {
  if ($ssh_dir != undef) {
    $dir = $ssh_dir
  } else {
  ...
  if ($dir != undef) {
    ...
    exec { "ssh_keygen-${name}":
      command => "/usr/bin/ssh-keygen -t ${ssh_keytype} -b ${bitlength} -f '${dir}/id_${ssh_keytype}' -N '${passphrase}' -C '${name}@${facts['networking']['fqdn']}'",
      user    => $name,
